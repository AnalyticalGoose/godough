...
func _init_schema(database_type: DatabaseType, database: Database) -> void:
    match database_type:
        DatabaseType.SETTINGS:
            ...
            database.add_valid_property("cam_zoom_speed", TYPE_FLOAT)
            database.add_valid_property("cam_smoothing", TYPE_FLOAT)
            database.add_valid_property("cam_mouse_sensitivity", TYPE_FLOAT)
...
func _load_settings() -> void:
    if not FileAccess.file_exists(SETTINGS_PATH):
        ...
        cfg_file.set_value("camera_settings", "cam_zoom_speed", 50.0)
        cfg_file.set_value("camera_settings", "cam_smoothing", 0.1)
        cfg_file.set_value("camera_settings", "cam_mouse_sensitivity", 0.25)
